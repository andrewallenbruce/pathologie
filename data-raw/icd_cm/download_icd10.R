...
webscrape_icd <- function(icd_version = "ICD10", year = NULL, quiet = FALSE) {
  ...
  if (icd_version == "ICD10") {
    ...
    if (is.null(year) | current_year) {
      path <- stringr::str_split(root_folders, "</A><br>")[[1]] |>
        stringr::str_extract_all("pub/Health_Statistics/NCHS/Publications/ICD10CM/\\d{4}/") |>
        purrr::compact() |>
        purrr::list_c() |>
        tibble::enframe() |>
        dplyr::mutate(year = as.numeric(stringr::str_extract(value, "\\d{4}"))) |>
        dplyr::filter(year == max(year)) |>
        dplyr::pull(value) |>
        as.character()

      recent_year <- stringr::str_extract(path, "\\d{4}")
      ...
      if (!all(file_match)) {
        ...
        download_exit_status <- try(
          download.file(file_zip, temp_file, quiet = FALSE)
        )
        ...
        file_name <- unzip(temp_file, list = TRUE) |>
          dplyr::filter(Name == paste0("Code Descriptions/icd10cm-codes-", file_year, ".txt") | Name == paste0("icd10cm-codes-", file_year, ".txt")) |> dplyr::pull(Name)

        unzip(temp_file, files = file_name, exdir = temp_dir, overwrite = TRUE)

        file_path <- file.path(temp_dir, file_name)

        icd_dictionary <- data.table::fread(file_path, sep = "\t", header = FALSE) |>
          data.table::setnames(old = "V1", new = "code_combo") |>
          as.data.frame() |>
          dplyr::mutate(
            code_combo = stringr::str_replace_all(code_combo, "\\s{2,4}", "_"),
            code_combo = stringr::str_replace_all(code_combo, "(?<=^[[:alnum:]]{7})\\s{1}", "_")
          ) |>
          tidyr::separate(code_combo, c("code", "description"), sep = "_") |>
          dplyr::mutate(
